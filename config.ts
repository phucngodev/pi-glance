...
/**
 * Set of segment IDs that are enabled by default.
 * When toggle-all is active, only these segments remain enabled.
 */
const DEFAULT_SEGMENT_IDS = new Set(DEFAULT_SEGMENTS.map((s) => s.id));

/**
 * Apply the toggle-all state to segments.
 * When enabled (ON): Only segments with default enabled=true remain enabled.
 * When disabled (OFF): All segments are disabled.
 */
/**
 * Set of segment IDs that have enabled=true in defaults.
 * When toggle-all is active, only these segments remain enabled.
 */
export const DEFAULT_ENABLED_IDS = new Set(
  DEFAULT_SEGMENTS.filter((s) => s.enabled).map((s) => s.id),
);
 ...
 * When disabled (OFF): All segments are disabled.
 */
export function applyToggleAll(config: GlanceConfig, enabled: boolean): GlanceConfig {
  const next = cloneConfig(config);
  for (const segment of next.segments) {
    // If toggle-all is ON, use default enabled state; otherwise disable all
    segment.enabled = enabled && DEFAULT_ENABLED_IDS.has(segment.id);
  }
  return next;
}
...
const PROVIDER_MODES = new Set<GlanceConfig["display"]["showProvider"]>([
  "auto",
  "always",
  "never",
]);
...
export function cloneConfig(config: GlanceConfig): GlanceConfig {
  return {
    ...
    git: { ...config.git },
    toggleAllEnabled: config.toggleAllEnabled,
  };
}
...
function normalizeConfig(raw: unknown): GlanceConfig {
  ...
  const record = raw as Record<string, unknown>;
  const editor =
    record.editor && typeof record.editor === "object"
      ? (record.editor as Record<string, unknown>)
      : {};
  const display =
  ...
  const toggleAll = record.toggleAll ?? defaults.toggleAllEnabled;
  ...
  return {
    ...
    display: {
      ...
      showProvider: parseStringEnum(
        display.showProvider,
        PROVIDER_MODES,
        defaults.display.showProvider,
      ),
    },
    ...
    toggleAllEnabled: parseBool(toggleAll, defaults.toggleAllEnabled),
  };
}
