...
import { collectGitSnapshot, nextGitRefreshDelay, parseGitStatus } from "../git.ts";
import type { GitConfig, GitSnapshot } from "../types.ts";
...
function assertRefreshDelays(): void {
  const repoSnapshot = parseGitStatus(
  ...
  assert.equal(
    nextGitRefreshDelay(repoSnapshot, { ...testConfig, pollIntervalMs: 10 }),
    1000,
    "repo min poll delay",
  );
