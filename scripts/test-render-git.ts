...
import type { GitSnapshot, GlanceState } from "../types.ts";
...
assert.equal(
  gitLine({ status: "dirty", dirty: true, unstaged: 1 }),
  "git main *",
  "dirty marker defaults on",
);
assert.equal(
...
assert.equal(
  gitLine({ status: "conflict", dirty: true, conflicts: 1 }, (config) => {
    config.git.showDirty = false;
  }),
  "git main !",
...
assert.equal(
  gitLine({}, (config) => (config.git.shaMode = "always")),
  "git main a1b2c3d",
  "sha always shows branch sha",
);
assert.equal(
